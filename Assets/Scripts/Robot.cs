...
public class Robot : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision) {
        ...
        if (direction == 0) animator.Play("Robot_Walk_Down");
        if (direction == 1) animator.Play("Robot_Walk_Left");
        if (direction == 2) animator.Play("Robot_Walk_Up");
        if (direction == 3) animator.Play("Robot_Walk_Right");
    }
}
